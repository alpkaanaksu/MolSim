...
Particle::Particle(const Particle &other)
        : x(other.x), v(other.v), f(other.f), old_f(other.old_f),
          ...
          type(other.type), directNeighbors(other.directNeighbors),
          diagonalNeighbors(other.diagonalNeighbors), avgBondLength(other.avgBondLength),
          stiffnessFactor(other.stiffnessFactor), pulled(other.pulled),
          id(other.id), fixed(other.fixed) {
...
Particle::Particle(std::array<double, 3> x_arg, std::array<double, 3> v_arg,
                   double m_arg, double eps, double sig, int type_arg, double avgBondLength_arg, int stiffnessFactor_arg, bool pulled_arg, bool fix)
        : x(x_arg), v(v_arg), m(m_arg), type(type_arg),
          f({0., 0., 0.}), old_f({0., 0., 0.}), epsilon(eps), sigma(sig),
          directNeighbors(), diagonalNeighbors(), avgBondLength(avgBondLength_arg), stiffnessFactor(stiffnessFactor_arg),
          pulled(pulled_arg), fixed(fix) {
    spdlog::debug("Particle generated!");
}
...
Particle::Particle(std::array<double, 3> x_arg, std::array<double, 3> v_arg, std::array<double, 3> f_arg,
        ...
        : x(x_arg), v(v_arg), m(m_arg), f(f_arg), old_f(old_f_arg), type(type_arg), epsilon(eps), sigma(sig), fixed(fix) {
    spdlog::debug("Particle generated!");
}
...
void Particle::setType(int type_arg) {
    type = type_arg;
}
...
std::string Particle::toString() const {
    ...
    stream << "Particle "<< id << ": X:" << x << " v: " << v << " f: " << f
...
void Particle::addDirectNeighbor(int id) {
    directNeighbors.push_back(id);
}

void Particle::addDiagonalNeighbor(int id) {
    diagonalNeighbors.push_back(id);
}
